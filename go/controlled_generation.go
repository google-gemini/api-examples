...
func JsonNoSchema() (*genai.GenerateContentResponse, error) {
	...
	prompt := "List a few popular cookie recipes in JSON format.\n\n" +
		      ...
		      "Return: list[Recipe]"
	response, err := client.Models.GenerateContent(ctx, "gemini-3-flash-preview", genai.Text(prompt), nil)
	if err != nil {
		log.Fatal(err)
	}
	printResponse(response)
...
func JsonEnum() (*genai.GenerateContentResponse, error) {
	...
	response, err := client.Models.GenerateContent(ctx, "gemini-3-flash-preview",
		contents,
		config,
	)
	if err != nil {
