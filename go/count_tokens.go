...
func TokensContextWindow() error {
	...
	modelInfo, err := client.Models.Get(ctx, "gemini-3-flash-preview", &genai.GetModelConfig{})
	if err != nil {
		log.Fatal(err)
	}
...
func TokensCachedContent() error {
	...
	cache, err := client.Caches.Create(ctx, "gemini-3-flash-preview", &genai.CreateCachedContentConfig{
		Contents: contents,
	})
	if err != nil {
		log.Fatal(err)
	...
	countResp, err := client.Models.CountTokens(ctx, "gemini-3-flash-preview", []*genai.Content{
		genai.NewContentFromText(prompt, genai.RoleUser),
	}, nil)
	if err != nil {
		log.Fatal(err)
	...
	response, err := client.Models.GenerateContent(ctx, "gemini-3-flash-preview", []*genai.Content{
		genai.NewContentFromText(prompt, genai.RoleUser),
